...
| TN-D2D-09 | γ-side spike packet wire surface is `{spike_count[3:0], spike_vec[7:0], gf_tag[3:0]}` routed by `src/d2d_holo_mesh.v`; per-direction TX/RX is 1 bit, NOT a Loihi-2-style graded payload. | `src/d2d_holo_mesh.v` ports/TX block, `docs/ARCHITECTURE_QUICK_WINS.md` QW-G-01 | `src/d2d_holo_mesh.v` (registered TX / latched RX) | future `docs/specs/SPIKE_PACKET_FORMAT_V0.md`; runtime witness via `sim/tb_canonical.v` neighbour-direction probes | RTL | Anyone documents the D2D path as a >1-bit-per-direction graded spike payload without an RTL change. |
...
| TN-TD-06 | A dedicated in-isolation FBB-ACTIVE testbench will assert FSM transition coverage and forbidden-state rejection (`test/tb_fbb_active_path.v` is stale today). | `docs/ARCHITECTURE_QUICK_WINS.md` QW-G-06, `TRIPLE_DECK_STATUS.md` §2.1 | `src/fbb_active_path.v` (RTL exists); testbench is stale | future repaired `test/tb_fbb_active_path.v` | SPEC | The repo claims FBB-ACTIVE in-isolation coverage exists today. |
